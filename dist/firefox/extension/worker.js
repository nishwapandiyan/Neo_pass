...
// Helper function to check if URL is injectable (not chrome://, chrome-extension://, etc.)
...
chrome.tabs.query({}, async tabs => {
    for (let tab of tabs) {
        if (!tab.url || !isInjectableUrl(tab.url)) continue;
        let url = tab.url;
...
function isLoggedIn(callback) {
    chrome.storage.local.get(['githubConnected'], function(result) {
        callback(!!result.githubConnected);
    });
}

// Function to prompt user to connect GitHub account
function showLoginPrompt(tabId) {
    showToast(tabId, 'Please connect your GitHub account in NeoPass popup first.', true);
}

chrome.contextMenus.onClicked.addListener((info, tab) => {
        if (!tab || !tab.url || !isInjectableUrl(tab.url)) return;
