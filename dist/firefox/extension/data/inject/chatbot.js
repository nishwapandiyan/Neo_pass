    ...
    chrome.storage.local.get(['stealth'], function(result) {
        ...
        function createChatOverlay() {
            ...
            overlay.style.cssText = `
                ...
                position: fixed;
                bottom: 20px;
            // Create chat overlay styled as a modern Gemini Sidebar
        ...
        <div style="display: flex !important; align-items: center !important; gap: 10px !important;">
            <div style="width: 30px !important; height: 30px !important; border-radius: 8px !important; background: linear-gradient(135deg, #6366f1, #a855f7, #ec4899) !important; display: flex !important; align-items: center !important; justify-content: center !important; color: white !important; font-size: 14px !important; font-weight: bold !important; box-shadow: 0 2px 8px rgba(99, 102, 241, 0.4) !important;">
                ✨
            </div>
            <div style="display: flex !important; flex-direction: column !important;">
                <span style="font-size: 15px !important; font-weight: 700 !important; background: linear-gradient(135deg, #818cf8 0%, #c084fc 50%, #f472b6 100%) !important; -webkit-background-clip: text !important; background-clip: text !important; color: transparent !important;">
                    Gemini Sidebar
                </span>
                <span style="font-size: 11px !important; font-weight: 500 !important; color: #9ca3af !important;">
                    Auto-Solver & Chat Assistant
                </span>
            </div>
        </div>
        <div style="display: flex !important; gap: 8px !important; align-items: center !important;">
            <button id="solve-screen-btn" style="background: linear-gradient(135deg, #6366f1, #8b5cf6) !important; color: #fff !important; border: none !important; border-radius: 6px !important; padding: 5px 10px !important; font-size: 11px !important; font-weight: 600 !important; cursor: pointer !important; display: flex !important; align-items: center !important; gap: 4px !important; transition: transform 0.2s ease !important;" title="Auto-solve question visible on screen">
                ⚡ Solve Screen
            </button>
            <span id="clear-chat" style="cursor: pointer !important; font-size: 12px !important; font-weight: 600 !important; color: #ef4444 !important; padding: 4px 6px !important; transition: opacity 0.2s ease !important;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'">Clear</span>
            <span id="close-chat" style="cursor: pointer !important; font-size: 20px !important; line-height: 1 !important; color: #9ca3af !important; transition: color 0.2s ease !important; font-weight: 500 !important; padding: 0 4px !important;" onmouseover="this.style.color='#fff'" onmouseout="this.style.color='#9ca3af'">×</span>
        </div>
        `;
        ...
        `;
            ...
            const solveScreenBtn = header.querySelector("#solve-screen-btn");
            if (solveScreenBtn) {
                solveScreenBtn.addEventListener("click", () => {
                    const questionData = extractCurrentQuestion();
                    if (questionData) {
                        const formatted = formatQuestionForChat(questionData);
                        if (formatted) {
                            window.autoSolvePrompt(formatted);
                            return;
                        }
                    }
                    // Fallback to active screen content
                    const pageText = document.body ? document.body.innerText.substring(0, 1500) : "";
                    if (pageText.length > 10) {
                        window.autoSolvePrompt("Solve this question visible on my screen:\n\n" + pageText);
                    } else {
                        addNotificationMessage("No question detected on screen");
                    }
                });
            }

            // Shared function to send chat messages
            async function submitMessage(messageText) {
                ...
                try {
                    ...
                    if (chatAboutQuestionEnabled && extractedQuestion && message !== extractedQuestion) {
            ...
            // Expose submit function for auto-solver
            window.autoSolvePrompt = async function(text) {
                const shadowHost = document.getElementById("chat-overlay-shadow-host");
        ...
        chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
            ...
            // Handle autoSolveQuestion action from runtime messages
            if (message.action === "autoSolveQuestion" && message.question) {
                if (typeof window.autoSolvePrompt === "function") {
                    window.autoSolvePrompt(message.question);
                }
            }
        });

        // Track last auto-submitted selection to avoid duplicate queries
        let lastAutoSubmittedText = "";
        let lastAutoSubmitTime = 0;

        // Auto-solve question whenever user copies or highlights a question text
        function checkAndAutoSolveSelection() {
            try {
                // Don't trigger if user is active inside the chatbot shadow DOM
                const activeElement = document.activeElement;
                const shadowHost = document.getElementById("chat-overlay-shadow-host");
                if (shadowHost && shadowHost.shadowRoot && activeElement && shadowHost.shadowRoot.contains(activeElement)) {
                    return;
                }
                ...
                // Require at least 15 characters to prevent accidental queries on small single words
                if (!selectedText || selectedText.length < 15) return;

                // De-duplicate fast repeated triggers (cooldown 4 seconds)
                const now = Date.now();
                if (selectedText === lastAutoSubmittedText || (now - lastAutoSubmitTime < 4000)) {
                    return;
                }

                lastAutoSubmittedText = selectedText;
                lastAutoSubmitTime = now;

                console.log("[NeoPass Auto-Solver] Text copied/dragged for solving:", selectedText.substring(0, 60) + "...");

                if (typeof window.autoSolvePrompt === "function") {
                    window.autoSolvePrompt(selectedText);
                }
            } catch (err) {
                console.error("Auto-solve selection error:", err);
            }
        }

        // Listen for copy event (when user hits Ctrl+C or copies text)
        document.addEventListener("copy", () => {
            setTimeout(checkAndAutoSolveSelection, 50);
        });
    });
})();
